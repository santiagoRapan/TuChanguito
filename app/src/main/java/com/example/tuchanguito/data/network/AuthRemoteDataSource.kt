...
class AuthRemoteDataSource(
    ...
    suspend fun login(email: String, password: String): AuthTokenDto =
        safeApiCall { api.login(com.example.tuchanguito.data.network.model.CredentialsDto(email, password)) }
    ...
    suspend fun register(
        ...
        surname: String,
        password: String
    ) = safeApiCall { api.register(RegistrationRequestDto(email, name, surname, password)) }

    suspend fun verify(code: String) = safeApiCall { api.verify(VerificationCodeDto(code)) }

    suspend fun changePassword(current: String, new: String) =
        safeApiCall { api.changePassword(PasswordChangeDto(currentPassword = current, newPassword = new)) }

    suspend fun recoverPassword(email: String) = safeApiCall { api.recoverPassword(email) }

    suspend fun resetPassword(code: String, password: String) =
        safeApiCall { api.resetPassword(PasswordResetDto(code = code, password = password)) }

    suspend fun getProfile(): UserProfileDto = safeApiCall { api.getProfile() }

    suspend fun updateProfile(name: String?, surname: String?): UserProfileDto =
        safeApiCall { api.updateProfile(UserUpdateRequestDto(name, surname)) }

    suspend fun sendVerification(email: String) = safeApiCall { api.sendVerification(email) }
}
